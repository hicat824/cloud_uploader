...
UPLOAD_DOCKER_IMAGE="127.0.0.1/kd-ad/oss_uploader_new:latest"
echo "更新本地docker......"
...
jq -c '.[]' $TASK_INFO_FILE | while read -r item; do
  ...
  docker run --rm \
    ...
    $UPLOAD_DOCKER_IMAGE \
    /bin/bash -c "umask 0000 && python3 /home/hadoop/bin/main.py -i ${SUB_TASK_INFO_FILE} -t ${DATA_TYPE} -m ${MODE} -s ${sn}"
